import { unreverseDomain, summarizeDnsLogs, summarizeRootDomains } from "../core/dns-logs.js";
...
export const fetchDnsLogSummary = async (client, { windowHours = DEFAULT_WINDOW_HOURS, topN = DEFAULT_TOP_N } = {}) => {
  ...
  // Root-domain rollup + GAFAM breakdown, computed over ALL resolved
  // queries (not just blocked) — mirrors the "Tên miền gốc" / "Sự thống
  // trị của GAFAM" sections of the reference dashboard.
  const allDomainCounts = rows.map((r) => ({
    domain: unreverseDomain(r.queryNameReversed),
    count: r.count,
  }));
  const { topRootDomains, gafamBreakdown, grandTotal } = summarizeRootDomains(allDomainCounts, topN);
  ...
  return {
    ...
    topRootDomains,
    gafamBreakdown,
    totalResolvedQueries: grandTotal,
  };
};
